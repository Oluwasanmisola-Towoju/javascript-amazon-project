...
export function renderOrderSummary() {
    ...
    cart.forEach((cartItem) => {
        ...
        // determine delivery option (use first option as default if missing)
    ...
    // allow changing delivery option from checkout page (saveToStorage already imported at top)

    document.querySelectorAll('.delivery-option-input').forEach((input) => {
        input.addEventListener('change', () => {
            const productId = input.name.replace('delivery-option-', '');
            const newOptionId = input.value;
    ...
    document.querySelector('.js-checkout-items-count').innerHTML = `Checkout (${cart.length} items) and (${cart.reduce((total, item) => total + item.quantity, 0)} products)`;
